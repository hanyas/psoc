...
def expectation(
    ...
    prior, closedloop, log_obsrv = \
        double_pendulum.create_env(params, eta)
...
def lower_bound(
    ...
    _, closedloop, log_obsrv = double_pendulum.create_env(params, eta)
    lls = double_pendulum.log_complete_likelihood(states, next_states, closedloop, log_obsrv)
    return - jnp.sum(lls)
...
def compute_cost(
    ...
    _, _, cost = double_pendulum.create_env(params, eta)
...
key, sub_key = jr.split(key, 2)
opt_state = create_train_state(sub_key, double_pendulum.network, 1e-4)

start = clock.time()

prior, closedloop, cost = \
    double_pendulum.create_env(opt_state.params, eta)
...
prior, closedloop, _ = \
    double_pendulum.create_env(opt_state.params, eta)
states = double_pendulum.simulate(prior, closedloop, nb_steps)
